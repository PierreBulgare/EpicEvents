...
class EventManager:
    ...
    def display_event_data(self, event: Evenement):
        ...
        print(
            f"{'Client':<30} "
            f"{TextManager.style(event.contrat.client.nom_complet, 'dim'):<30}"
        )
        print(
            f"{'Contrat':<30} "
            f"{TextManager.style(str(event.contrat.id), 'dim'):<30}"
        )
        print(
            f"{'Date de début':<30} "
        ...
        print(
            f"{'Commercial':<30} "
            f"{TextManager.style(event.commercial.nom, 'dim'):<30}"
        )
        print(
            f"{'Contact Support':<30} "
            f"{TextManager.style(event.support.nom if event.support else 'Aucun', 'dim'):<30}"
        )
        print(
        ...
        with self.db_manager.session_scope() as session:
            if not event_id:
                try:
                    WarningMessage.cancel_command_info
                    event_id = input(
                        "ID de l'évènement à supprimer : ").strip()
                    event = session.query(Evenement
                                          ).filter_by(id=event_id).first()
                    if not event:
                        ErrorMessage.data_not_found("Evenement", event_id)
